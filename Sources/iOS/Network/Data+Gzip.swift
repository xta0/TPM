  ...
  func gzipped(level: CompressionLevel = .defaultCompression, wBits: Int32 = Gzip.maxWindowBits + 16) throws -> Data {
    ...
    repeat {
      ...
      withUnsafeBytes { (inputPointer: UnsafeRawBufferPointer) in
        if let baseAddress = inputPointer.bindMemory(to: Bytef.self).baseAddress {
        ...
        data.withUnsafeMutableBytes { (outputPointer: UnsafeMutableRawBufferPointer) in
          ...
          if let baseAddress = outputPointer.bindMemory(to: Bytef.self).baseAddress {
            stream.next_out = baseAddress.advanced(by: Int(stream.total_out))
          } else {
            return
          }
    ...
    repeat {
      ...
      repeat {
        ...
        withUnsafeBytes { (inputPointer: UnsafeRawBufferPointer) in
          ...
          if let baseAddress = inputPointer.bindMemory(to: Bytef.self).baseAddress {
            stream.next_in = UnsafeMutablePointer<Bytef>(mutating: baseAddress).advanced(by: Int(inputStartPosition))
          } else {
            return
          }
      ...
      let inflateEnd = inflateEnd(&stream)
      // inflate returns:
      ...
      guard inflateEnd == Z_OK, status == Z_STREAM_END else {
        throw GzipError(code: status, msg: stream.msg)
      }
